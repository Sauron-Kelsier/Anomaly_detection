# BASED ON THE CLASSIFICATION OF THE THRESHOLDING MECHANISM, CREATES SUMMARY
# CREATES AN OUTPUT VIDEO BY CONCATENATING THE FRAMES IN THE ORIGINAL ORDER
...
######################################################################################################################	
	...
	for i in range(0,row):		
		if(output_val[i] >= threshold):

			index = (i * factor) + 1
			result[output_frame[i]-1] = 1
			image_loc = '{0}'.format(output_frame[i]+1+time_length) + '.jpg'
...
######################################################################################################################
	# setting up for video creation (GRAD-CAM VIDEOS)

	test_video_path = os.path.join(video_root_path,'{0}/grad_cam/{1}'.format(dataset,sys.argv[1]))

	count = 0
	output = sys.argv[1] + '_new_grad.avi'

	images = []

	for f in sorted(os.listdir(test_video_path)):	
		images.append(f)
...
######################################################################################################################	
	...
	# CONCATENATING TO CREATE A MOVIE
	
	for i in range(0,len(images)):
		image_loc = '{0}'.format(i+1) + '.png'
		image_path = os.path.join(test_video_path, image_loc )
		frame = cv2.imread(image_path)
		out.write(frame) # Write out frame to video	
	...
	f_measure = f1_score(ground_truth_val, result, average='weighted')
...
threshold = median_value +  (0.5 * standard_deviation_value)
